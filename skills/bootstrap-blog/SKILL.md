...
- **Reference image** (optional) — prompt: "Path to a reference image to anchor consistency? (Type a file path, or `skip` to add later.)" If supplied, store the *source* path now; Step 7 will copy it to `<target>/static/images/reference.png` and write that fixed destination into `metaphor.reference_image`.
