...
- **A mistyped `--config` or `--blog` no longer arrives as a stdlib traceback
  ...
  relative path *existed* but was resolved against the wrong directory. It left
  the step before that: a path that does not exist never reaches the renderer,
  so `yaml.safe_load(open(config))` raised `FileNotFoundError` and the operator
  got a traceback pointing into the stdlib. `update.py` now checks both paths
  up front and exits 2 naming the **resolved** path *and* the directory it was
  resolved from, since "a relative argument is not resolved where you assumed"
  is the whole subject of #59. Salvaged from #77.
...
> The sticker capability is off unless a blog asks for it, and the two engine
> knobs it adds are absent by default. `tests/unit/test_image_compose.py` — the
> cover-composition regression guard — passes **unmodified**. What *is* new
> behaviour is declared under **Changed**, including two silent failures made
> loud and two accepted divergences from the private implementation being
> retired: "no behavioural changes" means no *undeclared* ones.

### Added
- **Die-cut sticker sheets, generated by the same engine as covers
  (`features.stickers`, default OFF, config schema v7).** One blog carried a
  474-line private fork of `generate-images.py` for its sticker set — a second
  prompt-composition engine that had already drifted from the shared one, so
  every engine improvement landed on one side only. The fork is now portable
  data: six namespaced `sticker_*` layers, a `sticker` composition order, and 18
  entries in a dedicated `features.stickers.prompts_file`, composing prompts that
  are **byte-identical** to the pre-port ones (18 goldens derived from the
  private script's own `compose_prompt()`, asserted through the shipped engine —
  `tests/unit/test_stickers_golden.py`). Two scripts ship into
  `<site_dir>/scripts/` when the flag is on, and *nothing* when it is off:
  `generate-stickers.py` (a CLI-preserving shim: `--list`, `--only`, `--out`,
  `--dry-run`) and `build-sheets.py` — the only genuinely new code, and the only
  print-resolution layout in blog-craft. Sheets are A4 at exactly 300 DPI with a
  centred 3×3 grid, and the DPI is written **into** the PNG so printing at 100%
  maps the page 1:1 and the keyline stays a literal cut path. Page pixels are
  **derived**, never configured — `round(mm / 25.4 * dpi)` per axis, which is
  exactly `2480×3508` for A4 at 300 (verified against the shipped artifact's
  `IHDR` and its `pHYs 11811`); an explicit pixel key would let `size: a4` and the
  pixels disagree. `grid` is honoured rather than decorative (it also sets `pos`'s
  valid range), an unknown `size` is a hard error rather than a silent fallback to
  A4, and the filename prefix comes from `project.name` so the shipped script is
  not one blog's shape. No Hugo surface at all — no page, shortcode, gallery or
  CSS: a sticker set is a private **print** asset. `migrations/006_to_007.py`
  seeds `features.stickers: {enabled: false}` for existing blogs and leaves an
  explicit `true` alone. See `docs/CONFIG.md` §13.
- **`_template` on a layer — a structural prose frame (`compose.py`).** A dict
  layer may declare `_template: "… {} …"`, applied to whatever the layer resolved
  to (a table hit or a free-form passthrough). It exists because the sticker mood
  was a code-level `f"Frank's expression: {mood}."` with no expression in config,
  and baking the prefix into 18 entries would let a 19th silently omit it. A layer
  resolving to `""` stays `""`. The validator requires exactly one `{}` and **no
  other brace anywhere** — stricter than "one placeholder", by policy, so a frame
  has one obvious spelling and can be emitted mechanically; `{0}`, `{name}`,
  `{:>10}` and the `str.format` escapes `{{`/`}}` are all rejected even though
  they format fine. Prose containing braces is *not* a hazard: only the template
  is parsed. **Never put `_template` on a shared layer whose values are already
  complete sentences** — it would double-frame every cover composing through it,
  silently; `validate_config.py` additionally rejects an order token `X[y]` whose
  layer `X` declares a frame, because that path is deliberately unframed and the
  two spellings are otherwise indistinguishable in config.
- **`--out <dir>` — a non-destructive generate mode for `generate-images.py`.**
  A data-loss guard, not a convenience flag: the engine writes the last variant
  straight to the entry's `output:`, while the sticker workflow is *generate
  candidates, eyeball them, copy a winner over the master by hand*. Porting the
  stickers as ordinary entries would have overwritten the curated masters the
  whole print workflow exists to protect. Under `--out` the entry's `output:` is
  never written (its parent directories are not even created), `post_process` is
  skipped (its steps target the *published* asset), and a `DIR` that would alias
  any selected entry's `output:` — including through a symlink or a hard link —
  **refuses the whole run** before the first API call, because a path-shaped
  promise is not a guarantee. Filenames are the **basename of the entry's
  `output:`**, with *every* member of a colliding group renamed
  `<key>-<basename>` when two or more selected entries share one: `<key>.png`
  would have lost the established `sticker-<key>.png` naming the runbook depends
  on, and the bare basename alone would have collapsed **85 of 91 cover entries**
  (all ending `/cover.png`) into a single file. The decision is a function of the
  selected set, not of iteration order. `--dry-run` names the `--out` destination
  rather than the `output:` path it will never touch. Generalises usefully to
  covers: "preview without clobbering the shipped one".
- **`image.fallback_model` and `image.timeout_ms`.** The retired script retried a
  second model on any exception and capped HTTP at 120 s — the fallback exists
  precisely because the primary is a *preview* model. Both are now engine-wide and
  both are absent by default, so a blog that sets neither generates exactly as
  before. The fallback is attempted when the primary raises **or** returns a
  response with no image part, and it adds a retry without ever softening a hard
  failure: if every configured model raises, the last exception **propagates**
  (with no fallback set, the single attempt propagates unchanged). An image-less
  response stays a soft failure (`rc=1` for that key, nothing written), and the
  mixed case — primary raises, fallback answers imagelessly — propagates the
  primary's exception rather than reporting "the model declined" for what was a
  transport or auth error. Each attempt logs its model **and the exception type**,
  since many SDK exceptions have an uninformative `str()`.
- **`tools/migrate_stickers.py` — the one-time transform `/update` cannot do.**
  ...
  would then lose its clothing sentence *entirely* (~85 of 90 covers in the blog
  being migrated); when the layer exists the key is omitted and the tool
  **checks**, through the shipped `resolve_layer`, that the existing layer still
  returns every sticker's prose unchanged. It never emits a bare `mood:`, warns
  about any emitted key the config already defines, and refuses an empty
  `clothing`/`mood`/`scene` (the legacy join did not filter empty sections while
  `compose()` does, so a blank field would quietly compose one section fewer), a
  duplicate sticker key, a configured path escaping the blog root, and a
  `prompts_file` that resolves to the legacy source itself — writing there would
  destroy the only copy of the prose being migrated.
- **`/update` retires the two private script copies.** New
  `manifest.legacy_dests` rows map `scripts/build-sheets.py` and
  `scripts/generate-stickers.py` to their `_private/` predecessors. Both are
  `framework`, so the planned action is **`replace`**, not `relocate`:
  blog-craft's copy wins, the private one is unlinked, and any local edits to
  those two scripts are discarded — deliberately, since the port supersedes them
  (only `merged`-class files preserve operator edits). The **private directory
  itself survives** the update while it still holds content; emptying it is the
  operator's own last step. A blog that never had those paths has no legacy side
  to find, so nothing is relocated and nothing is deleted; with the feature
  disabled the scripts are not even staged.

### Changed
- **A duplicate `(sheet, pos)` in the sticker prompts file is now a hard error
  naming both stickers and the cell.** This **is** a behaviour change against the
  code being retired, which did `sheets[sheet][pos] = key` — last-wins, so one
  sticker silently vanished from a page the operator *prints and cuts*: paper, ink
  and manual cutting spent on a sheet with a hole in it, with no warning
  anywhere. It is an *unobservable* change for the data being ported: those 18
  stickers occupy 18 distinct `(sheet, pos)` pairs (measured, and pinned by a test
  that builds both sheets from the real vendored placements). The same `pos` on a
  different sheet stays legal. `pos` outside the grid, and a missing image for a
  placed sticker, were already errors; this closes the last silent hole in that
  class.
- **An entry whose composed prompt is empty now prints a `WARN` naming the key
  instead of being skipped in silence.** Pre-existing behaviour on the **cover**
  path as well as the sticker one: an unresolvable `composition.order` — e.g. a
  bare `sticker` where the engine matches only `composition_orders[sticker]` —
  composes an empty prompt, and the entry dropped out with **no output and exit
  code 0**. Measured during this work: all 18 stickers, nothing generated, a
  successful-looking run. **The exit code is deliberately unchanged**: an operator
  may have a legitimately empty entry, and making it non-zero would be a real
  behaviour change on the path every blog takes. Loud, not fatal.
- **Two accepted divergences in the sticker regen workflow, declared rather than
  hidden.** (1) The **run-level contact sheet** — one sheet per run across the
  keys generated, written to `<out>/contact-sheet.png` whenever at least two keys
  succeeded — is built by the shim, not the engine, because the engine's sheet is
  per *key* across that key's variants and only fires on `--count > 1`, which a
  one-image-per-key sticker run never reaches. Its column count and tile width are
  the originals (`cols=5, tile_width=420`), but its **layout** is blog-craft's
  (label at the top of a fixed tile) rather than the private helper's
  ...
  root** — `<repo>/regen/` by default. That is a runbook change; pass
  `--out blog/_private/stickers/regen` to keep the old location.
- **`generate-stickers.py --dry-run` prints the FULL composed prompt.** The
  private version truncated at 300 characters, which is why its goldens had to be
  derived from `compose_prompt()` rather than from its CLI. The truncation was a
  defect and is not preserved.
...
- **`curation.archive_cap` pruned the variants of its own `--count N` run.**
  Pre-existing, and on the **default cover path**, not the sticker one:
  `write_archive_entry` applies the FIFO cap on every call, so a `--count N` run
  with `archive_cap < N` deleted its own earlier variants while still in flight —
  after which the contact sheet opened files that no longer existed and the run
  died with `FileNotFoundError` inside PIL. It survived in practice only because
  the variant the `output:` write reads is the newest and therefore never pruned.
  The variant list is now filtered to files that still exist, with a warning on
  stderr naming `archive_cap`, and the run proceeds with the survivors.
- **Sticker generation in the consumer blog had been broken since its own cutover
  to blog-craft, not merely duplicated.** Its `generate-stickers.py` imported
  `lib.contact_sheet` at module level, and that file was deleted by the cutover
  commit — so `ModuleNotFoundError` fired before argument parsing and not even
  `--list` worked. `build-sheets.py` did not import it, which is why the breakage
  went unnoticed: the visible artifact still rebuilt from already-generated
  images. The baseline for "no behavioural changes" is therefore the composition
  rule **as written in its source** — intact and fully readable, and what the 18
  goldens pin — not the runtime behaviour of a script that crashes.

### Documentation
- `docs/CONFIG.md` gains **§13**: every `features.stickers` key with its default,
  the default-OFF rationale, the derived page geometry and why it is not a linear
  scale, the sticker composition order with the border-after-scene note, the
  `_template` directive with the do-not-share-a-layer warning, the placement
  guards, the `--out` contract, the two reliability knobs, and the adoption
  runbook. `image.fallback_model` / `image.timeout_ms` are documented in the
  `image` block, and the document is corrected to **v7** — its title, its sample
  and its accepted-versions line were all stale at v6.
- `MEDIA-GUIDE.md` gains a short "Sticker sets" note: the two scripts, the
  pick-a-winner-then-copy flow, and print at 100%.
